...
| Cognition / knowledge | `docs/cognition/`、`knowledge/` 脚手架与本地私有条目 | 推理纪律、签名和弱点接地 | 不成为盲扫 playbook、确认依据或公开语料包 |
...
锁，再获取 assignment 锁；只有 cancellation 与 transcript-proven interrupted-Reviewer
recovery 事务允许按唯一 `runtime -> assignment` 顺序嵌套，以冻结新 Stop/child activity 与
assignment reset 之间的竞态，任何反向顺序都禁止。Agent
...
Reprojection 对 validated snapshot 只构造一次 Agent attempt graph；同一 parent transcript 的
所有 tool/lifecycle token 合并为一次流式扫描，child-claim causal owner 也缓存同一 parent 的
Agent tool-use index。不得在每条 lifecycle record 内重新读取整本 journal、重建全部 attempts
或逐 token 重扫大型 transcript；否则 Start hook 会在 receipt 已提交后卡在 derived projection，
制造不可见的半启动窗口。
...
| `tools/runtime_receipts.py` + Root/Agent/foreign-lifecycle/interrupted-Reviewer receipt schemas | 含 instruction-bundle digest 的 plan-bound type+prompt、typed tool-call/request budget 唯一 formatter、Start 冻结与 child PreToolUse 重验/原子 claim，assignment-free global completion formatter/lifecycle、exact child-sidechain transcript truth、hook-observed launch/return/failure、foreign Claude lifecycle admission/quarantine、pre-model Reviewer Start supersession、batched transcript projection、immutable result、ROOT_DIRECT claim/terminal projection | workers、instruction bundle、turn contract、parent/child transcript、run model、loop journal、schema conformance |
...
| `knowledge/` | 公开仓库只保存 README/模板脚手架；grounding 与 weaponized 实际条目均留本地 | `.gitignore` 默认拒绝条目上传；knowledge checks 验证本地 grounding 结构 |
...
- Date: 2026-08-01
- Scope: `Xunji-Harness` 的公开 `knowledge/` 只保留 README、grounding 模板、通用
  `_lexicon.md`、weaponized README 与空目录占位；grounding/weaponized 实际条目改为默认 gitignored。
  同步 Claude/Codex knowledge owner、cognition/workflow、工具说明和生成模板，删除“公开语料层”
  的旧承诺。远端通过无父提交的干净快照替换含未脱敏知识条目的历史分支。
- Architecture impact: knowledge persistence/publication ownership changed — 运行时仍按
  `live fingerprint -> local knowledge -> hypothesis -> evidence gate` 使用本地条目，但公开仓库
  不再拥有或分发实际知识语料；`.gitignore` 是防重复上传的机械边界。
- Owner/enforcement: `knowledge/README.md` 拥有本地层与公开脚手架契约；`.gitignore` 默认拒绝
  `knowledge/*.md` 实际条目；`tools/check_knowledge.py` 只验证本地 grounding 条目结构，不构成发布许可。
- Compatibility/migration: 新 clone 得到空知识库脚手架；既有操作者本地条目继续可被 matcher
  读取，但重新加入 Git 必须显式越过 ignore，视为违反发布契约。无 run/schema/receipt 迁移。
- Verification: knowledge/rule/template/runtime-boundary/selftest、tracked knowledge allowlist、
  sensitive-pattern 文件级扫描、远端 ref/tree 校验与独立复审。
- Independent review: 见 `review/records/2026-08-01-knowledge-scaffold-sanitization.md`；记录绑定
  最终 maintenance diff，历史替换另以远端 before/after ref 和 root-commit parent count 验证。
- Exclusions: 本地私有知识文件不删除、不上传；不改变 live target、run/canonical state、Hook、
  guard、privacy 或 evidence gate 行为。GitHub cached views、PR refs、forks 和外部 clone 需按平台
  流程另行清理，不能由一次 force-push 保证物理删除。

### 2026-08-01 — Harness positioning and README simplification

- Date: 2026-08-01
- Scope: 将公开项目定位从“自主红队工作区 / red-team toolkit”统一为“面向 Claude Code
  的渗透 / 红队 Harness”；中英文 README 以 `/loop <source>` 作为持续循环入口，并保留“对
  `<target>` 进行渗透”作为单周期自然语言入口，明确客户端转发边界与两者语义差异；再以简短
  设计原则、带 `cycle_end` 回边和 closure 出口的字符运行流、边界和 owner 链接说明项目。删除工具直调、安装、自检、Prompt
  模板集合、内部架构长文和项目地图，并用对称 badge、副标语、引语、表格与字符架构图改善公开入口。
  同步 `CLAUDE.md`、`docs/ROUTER.md` 与 `pyproject.toml` 的描述字段，消除公开入口术语漂移。
- Architecture impact: none — 本轮只澄清既有 Agent Harness 身份并重组公开文档层级；
  Root/Agent/Reviewer 角色、authority、state ownership、Tool contract、lifecycle、safety、
  evidence、review、closure 和 runtime behavior 均未改变。
- Owner/enforcement: `docs/ARCHITECTURE.md` 继续拥有项目定位与架构真值；`CLAUDE.md`
  拥有 Claude 主驾驶角色；`docs/ROUTER.md` 拥有指引路由。README 只做简短入口和 owner 链接，
  不再复制完整运行契约。
- Compatibility/migration: 无 schema、run、receipt、claim、journal、pointer、配置格式或代码迁移；
  `pyproject.toml` 仅修改人类可读的 description。
- Verification: `git diff --check`、定位术语扫描和 README 相对链接目标检查通过；
  `tools/check_rules.py`、`tools/check_templates.py` 与 `tools/check_runtime_boundary.py` 均通过。
- Independent review: 当前最终 diff 由 fresh-context Claude Code 独立复审；初审唯一 WARN
  是中英文字符运行流的回环边框与 artifact 合流箭头未对齐，修正后的增量复核、driver disposition
  与当前 fingerprint 绑定记录见 `review/records/2026-08-01-readme-loop-positioning-review.md`。
- Exclusions: 不修改 Hook、guard、privacy、sentinel、工具实现、run/canonical state、历史 review
  record、`.agents/skills`、用户现有 `AGENTS.md`/project-intro 改动或目标/evidence artifacts。
