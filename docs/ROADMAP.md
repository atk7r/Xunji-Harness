...
2. Every item must pass the project's three tests, or it does not ship:
   ...
   - **attacker, not a scanner** — payload knowledge is used per-target through the
     evidence gate, never fired as a blind checklist; the public repository ships
     only the knowledge scaffold, while grounding and weaponized entries stay local
...
- **Status (2026-06-17)**: the flywheel is now end-to-end. Write end = check_run
  ...
  surfaces recognition + anchors only (local grounding has no payloads — `check_knowledge`
