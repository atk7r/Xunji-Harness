...
All output for one target is self-contained under a single run directory. The AI
never writes target output anywhere else — no separate top-level reports or poc
folders. One target, one directory.
...
runs/<target_slug>_<YYYYMMDD>/
  ...
  review.md
  report.md
  artifacts/        # raw captured evidence: saved responses, headers,
                    #   request/response pairs, screenshots. Referenced by E-IDs
                    #   in evidence.md. Keep only what a finding needs.
  poc/              # reproduction PoC for a CONFIRMED finding in this target
                    #   only. One subfolder or file per finding, named by the
                    #   hypothesis / finding id. Safe, minimal, non-destructive.
```

Rules for the output folders:

- `artifacts/` and `poc/` are created lazily — only when there is evidence to
  store or a confirmed finding to reproduce. An empty run has neither.
- Every artifact must be traceable from an `evidence.md` entry (its `E-ID`), and
  every `poc/` item from a confirmed `report.md` finding. No orphan files.
- `poc/` holds per-target reproduction only. It is not a general exploit,
  payload, or scanner library — see Repository Discipline in `CLAUDE.md`.
...
summarizing instead of advancing a front. DeepSeek-only runs should use
