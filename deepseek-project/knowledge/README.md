...
This base belongs to the DeepSeek workspace (`deepseek-project/`) and is
**independent** from the parent vulnfinder workspace's `knowledge/`. The two
baselines are kept separate by design — DeepSeek builds and curates this base
itself; do not share, symlink, copy entries from, or read across the boundary.
The scaffolding (this contract, `_TEMPLATE.md`, `tools/check_knowledge.py`) is
shared; the entries are not.
