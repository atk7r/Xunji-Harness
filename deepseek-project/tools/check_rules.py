...
SKIP_DIRS = {
    ...
    "reports",
    "deepseek-project",
    # runs/ is sanctioned per-target output (incl. runs/<t>/poc and artifacts).
    # Reproduction PoCs for confirmed findings are legitimate output, so the
    # forbidden-path/text scan does not police inside runs/. The 9 state files
    # are validated separately by check_run.py.
    "runs",
}
...
REQUIRED_FILES = [
    ...
    Path("docs/deepseek/README.md"),
]

REQUIRED_TEXT = {
    # Guard the run-time authority boundary: the autonomous driver must not
    # rewrite its own guardrails mid-run. (Native framing — this IS DeepSeek's
    # project, but project-rule changes are a separate maintenance activity.)
    Path("docs/deepseek/README.md"): [
        "autonomous driver must not rewrite its own",
        "During a run you own the run-level output only",
    ],
    Path("docs/ROUTER.md"): [
        "autonomous driver must not rewrite its own guardrails",
