...
- Knowledge = **use payload knowledge to attack** (a reasoning attacker, not a payload
  scanner). The public repository ships only the `knowledge/` contract and templates;
  operator-local, gitignored grounding entries provide recognition signatures, weak-point
  anchors and verification notes, while local `knowledge/weaponized/` holds payloads,
  chains and PoCs. The forbidden thing is the **blind
