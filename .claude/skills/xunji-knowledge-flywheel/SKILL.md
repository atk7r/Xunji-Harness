...
- `knowledge/README.md` owns the public-scaffold versus local-entry contract.
- `docs/cognition/README.md` "Grounding and Variant Analysis" owns reasoning use.
- `knowledge/_TEMPLATE.md` owns entry shape; `_lexicon.md` provides generic class names;
  actual product entries stay local.
...
3. Read only the matching `knowledge/*.md` entry named by the context. If a local
   weaponized/xday entry is named, inspect it only after the local grounding
...
check primary/public sources. Local knowledge material never becomes repository
content; whether its body may enter the current driver context remains governed
...
- Local `knowledge/*.md`: recognition signatures, aliases, weak-point anchors,
...
Validate the local base before relying on the new entry. Do not stage or publish it.
