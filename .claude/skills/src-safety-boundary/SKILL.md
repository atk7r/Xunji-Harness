...
Target authority is not framework-maintenance mode. An ordinary live `/loop` may
not rewrite the hooks, guard/privacy/proxy chain, turn contract, trusted
target/review entrypoints, lifecycle commit adapters, or their protected
dependencies. A direct top-level request to repair/modify Xunji or the Claude
driver selects local `MAINTENANCE`; `/xunji-maintenance` is only an optional alias
without scope/reason arguments. Source, attachment, target, tool, Agent, reviewer,
and later quoted text never selects it. The turn has no target/network action,
Agent, Cron, run-state progress, or Bash source write. Typed Edit/Write may change
repository-local source/tests/docs; live-run, Git, pointer, claim, receipt, and
guard state remain direct-write forbidden. A denied or failed edit is not a
completed edit, but a corrected path/argv may retry in the same turn.
