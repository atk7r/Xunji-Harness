...
- Evidence: Runtime artifacts use `output_gate.diff`, `run_gate.hunks-01.diff`, `run_gate.hunks-02.diff`, and `settings.diff`; closure artifacts use `check_run.hunks-01.diff` through `check_run.hunks-04.diff`.

## PR-002 - No runtime execution observation

- Status: accepted
- Resolution: Added and executed an isolated installed-entrypoint observation. All 19 checks pass: explain/pause write denial, pre-Agent target denial, unmerged disposition denial, post-merge target allowance, Cron list/create/delete/list ownership, foreground review timing, weak/structured completion responses, and a ten-event valid receipt chain.
- Evidence: Runtime-scope `runtime_observation.py`, `runtime_observation.lines-001-190.txt`, `runtime_observation.lines-191-380.txt`, `runtime_observation.lines-381-end.txt`.
...
- Evidence: Parent `peer_review.round1.md`, `peer_review.round1.json` and runtime-scope observation artifacts.
...
- Evidence: Runtime-scope `installed-settings.json`, `settings.diff`, observation artifacts, and `selftest_all.log`.
