...
- 只读【本地接地层】`knowledge/*.md`(非递归 → `knowledge/weaponized/` 子目录天然不入); 显式拒读 weaponized。
- 只吐识别签名 + 弱点锚(class+mechanism+CVE); 接地层无 payload/steps(check_knowledge 已强制)。
...
def load_entries(kb_dir: Path = KB) -> list:
    """读本地接地层 `*.md`(非递归 → 不含 weaponized/)→ [Entry]。跳过 README/_TEMPLATE/_lexicon。"""
    out: list = []
    if not kb_dir.is_dir():
        return out
    for f in sorted(kb_dir.glob("*.md")):          # 非递归: weaponized/ 子目录天然不入
        if f.name in ("README.md", "_TEMPLATE.md", "_lexicon.md"):
            continue
...
def grounding_body(path: Path) -> str:
    """frontmatter + 起始 HTML 注释之后的正文 = 本地接地内容(check_knowledge 保证无 payload)。"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="指纹飞轮检索端: 据目标命中 knowledge 条目并吐弱点锚(只读本地接地层)")
...
def _selftest() -> int:
    ...
    # 公开脚手架(必须跳过，即使它意外带 entry-like frontmatter)
    (kb / "README.md").write_text("# readme\n", encoding="utf-8")
    (kb / "_TEMPLATE.md").write_text("---\nid: tpl\nsignatures: [\"x\"]\n---\n", encoding="utf-8")
    (kb / "_lexicon.md").write_text(
        "---\nid: lexicon\nsignatures: [\"generic-class\"]\n---\n", encoding="utf-8")
    ...
    checks = [
        ("加载本地 grounding 条目 foobar-cms", "foobar-cms" in ids),
        ("排除 weaponized/ 子目录条目", "foobar-weap" not in ids),
        ("跳过 README/_TEMPLATE/_lexicon 脚手架",
         "tpl" not in ids and "lexicon" not in ids and len(ents) == 1),
