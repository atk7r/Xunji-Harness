...
def load_events(run_dir: str | Path) -> list[dict]:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        try:
            ...
            out.append({"schema": SCHEMA, "event": "corrupt_line", "raw": line[:200]})
    return out
...
def append_event(run_dir: str | Path, event: str, *, note: str = "", data: dict | None = None) -> dict:
    run_dir = _resolve_run_dir(run_dir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.event == "status":
        ...
        try:
            rec = append_event(args.run_dir, event, note=args.note, data=payload)
