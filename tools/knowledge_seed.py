...
- 只产【本地接地层】骨架(recognition + 弱点锚 + 校验原则 + 混淆项 + 引用) —— 绝不产 payload/exploit/PoC
...
def extract_signatures(body: str, limit: int = 6) -> list[str]:
    ...
    # 否则抽到的 signature 可能把 payload 形状串写进接地层 —— 接地层只该有识别特征, 不该有武器形状)
    seen: list[str] = []
...
<!--
SEED scaffold (knowledge_seed.py). LOCAL grounding tier — gitignored by default.
...
def _selftest() -> int:
    ...
    checks.append(("grounding 层无 payload/exploit 标题(分层路由正确)",
