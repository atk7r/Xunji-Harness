...
def _allowed_hosts(run_dir: Path) -> set:
    """从 run 的授权资产台账(coverage.json assets + target/surface 里的域名)提 host 白名单 ——
    防篡改/越界录像让 replay 打 scope 外(dogfood 第6次 BLOCKER: replay 命令行只有 run 路径,
    真 URL 在录像里, hook 的 scope 拦截看不到)。"""
    hosts: set = set()
    for cov in run_dir.glob("**/coverage.json"):
        try:
            d = json.loads(cov.read_text(encoding="utf-8", errors="replace"))
            for a in d.get("assets", []):
                h = a.get("host") if isinstance(a, dict) else None
                if h:
                    hosts.add(str(h).lower())
        except Exception:
            pass
    for name in ("target.md", "surface.md", "surface_recon.md"):
        p = run_dir / name
        if p.is_file():
            for m in re.findall(r"https?://([a-z0-9.\-]+)",
...
def replay_one(rec_path, *, force: bool = False, timeout: int = 20,
    ...
    if allowed_hosts is not None and host and host not in allowed_hosts:
        ...
        out["note"] = f"host {host} 不在 run 授权资产台账 —— 拒绝重放(防篡改录像打 scope 外)"
        return out
...
def replay_run(run_dir, *, force: bool = False, timeout: int = 20,
    ...
    if allowed_hosts is None:
        allowed_hosts = _allowed_hosts(run_dir)   # 默认从该 run 的授权台账提 host 白名单
...
def main() -> int:
    ...
    ap.add_argument("--scope", default=None,
                    help="逗号分隔 host 白名单(覆盖从 run 台账自动提取); 单文件模式建议显式给")
    ...
    if t.is_file():
        if scope is None:
            print("[warn] 单文件模式无 run scope 上下文 —— 不做 host 白名单检查, 篡改录像会打其 url。"
                  "需要 scope 限制请用 --scope。", file=sys.stderr)
...
def _selftest() -> int:
    ...
    try:
        ...
        checks.append(("正确录像 -> IDENTICAL",
                       replay_one(mkrec("a.html.replay.json", "GET", url, 200, sha))["verdict"] == "IDENTICAL"))
        # status 同 sha 异(内容动态变) -> CONSISTENT
        checks.append(("status同 sha异 -> CONSISTENT",
                       replay_one(mkrec("b.html.replay.json", "GET", url, 200, "0" * 40))["verdict"] == "CONSISTENT"))
        # status 异(当时声称404实际200) -> DIVERGED
        checks.append(("status异 -> DIVERGED(证据存疑)",
                       replay_one(mkrec("c.html.replay.json", "GET", url, 404, sha))["verdict"] == "DIVERGED"))
        # 写操作 -> SKIPPED-WRITE; --force 才重放
        post = mkrec("d.replay.json", "POST", url, 200, sha, bodyval="x")
        checks.append(("写操作默认 SKIPPED-WRITE", replay_one(post)["verdict"] == "SKIPPED-WRITE"))
        checks.append(("--force 重放写操作(不再skip)", replay_one(post, force=True)["verdict"] != "SKIPPED-WRITE"))
        # 不可达 -> UNREACHABLE
        checks.append(("目标不可达 -> UNREACHABLE",
                       replay_one(mkrec("e.replay.json", "GET", "http://127.0.0.1:1/", 200, sha),
                                  timeout=2)["verdict"] == "UNREACHABLE"))
        # 坏记录 -> BAD-RECORD
        bad = d / "bad.replay.json"
        bad.write_text("{not json", encoding="utf-8")
        checks.append(("坏 JSON -> BAD-RECORD", replay_one(bad)["verdict"] == "BAD-RECORD"))
        # 破坏性 DELETE -> SKIPPED-DESTRUCTIVE(--force 也不解锁)
        dele = mkrec("del.replay.json", "DELETE", url, 200, sha)
        checks.append(("DELETE -> SKIPPED-DESTRUCTIVE", replay_one(dele)["verdict"] == "SKIPPED-DESTRUCTIVE"))
        checks.append(("DELETE --force 仍 SKIPPED-DESTRUCTIVE",
                       replay_one(dele, force=True)["verdict"] == "SKIPPED-DESTRUCTIVE"))
        # scope 白名单(防篡改录像打 scope 外)
        ...
        try:
            checks.append(("guard 异常 -> GUARD-BLOCKED(不崩)",
                           replay_one(mkrec("g.html.replay.json", "GET", url, 200, sha))["verdict"] == "GUARD-BLOCKED"))
        finally:
            probe.send = _orig
        # 批量(显式 scope)
