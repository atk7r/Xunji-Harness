...
本工具是 grounding 端 knowledge_match 的【镜像对称】: 同一套 signatures 匹配, 但读【本地 xday 库】。
...
import knowledge_match   # 复用本地签名匹配(weaponized 与 grounding 共用同一套 signatures)

KB = ROOT / "knowledge"
WEAP = KB / "weaponized"
XDAY = ROOT / "poc_library" / "xday"

_KN_REF = re.compile(r"knowledge/([A-Za-z0-9_-]+)\.md")   # xday README 里指向本地 grounding 条目的引用
...
def load_local_stores(xday_dir: Path = XDAY, weap_dir: Path = WEAP) -> dict:
    ...
    # ② knowledge/weaponized/*.md —— 武器化笔记/payload/链(与本地 grounding 同 signatures)
    if weap_dir.is_dir():
...
def match(body_text: str, kb_dir: Path = KB, xday_dir: Path = XDAY,
    ...
    """目标 body → 本地 grounding 签名命中 knowledge id → 仅返回【有本地 xday 存货】的 (id, sigs, stores)。"""
...
def _selftest() -> int:
    ...
    # 本地 grounding 条目(有 signatures, 供匹配)
    (kb / "foobar-cms.md").write_text(
        "---\nid: foobar-cms\nproduct: FooBar CMS\nmaturity: seed\n"
        'signatures: ["foobar-cms", "/fb/login.do"]\n---\n\n## Recognition\n- x\n', encoding="utf-8")
    # 本地 grounding 条目(无本地 xday, 不该被 xday_match surface)
    ...
    checks = [
        ...
        ("grounding 命中但【无本地货】→ 不 surface(nostore)", body_nostore == []),
