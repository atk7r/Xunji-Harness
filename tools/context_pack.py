...
def _knowledge_xday_summary(kb_ids: list[str], *, kb_dir: Path | None = None,
    ...
    for kid in kb_ids[:6]:
        ...
        if e:
            ...
            lines.append(f"- knowledge `{kid}`: id referenced by front/coverage but no local grounding entry found")
        ...
        if local:
            ...
            lines.append("  - local xday: withheld until a local grounding entry matches")
    return lines
...
def _selftest() -> int:
    ...
    (weap / "missing-public.md").write_text(
        "---\nid: missing-local\n---\nmust stay hidden without local grounding\n",
        encoding="utf-8")
    xp = xday / "foobar-cms"
    xp.mkdir()
    ...
    checks = [
        ...
        ("local xday pointer requires a matching local grounding entry",
         "withheld until a local grounding entry matches" in missing_public_summary
